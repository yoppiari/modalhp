<template>
  <div class="bg-green-50 min-h-screen pb-24">
    <div class="bg-green-600 text-white p-6 rounded-b-[2.5rem] shadow-lg mb-6 sticky top-0 z-10">
       ...
       <div class="bg-white p-5 rounded-2xl shadow-sm border border-green-100 space-y-4">
          ...
          <div>
            ...
            <div class="grid grid-cols-3 gap-2">
               <button @click="commission = 20" :class="commission === 20 ? 'bg-green-600 text-white ring-2 ring-offset-2 ring-green-600' : 'bg-slate-100 text-slate-600'" class="py-2 rounded-lg font-bold text-sm transition-all">
                  GoFood
                  <span class="block text-[10px] font-normal opacity-80">20%</span>
               </button>
               <button @click="commission = 20" :class="commission === 20.001 ? 'bg-green-600 text-white ring-2 ring-offset-2 ring-green-600' : 'bg-slate-100 text-slate-600'" class="py-2 rounded-lg font-bold text-sm transition-all" @click.prevent="commission = 20">
                  Grab
                  <span class="block text-[10px] font-normal opacity-80">20%</span>
               </button>
               <button @click="commission = 15" :class="commission === 15 ? 'bg-green-600 text-white ring-2 ring-offset-2 ring-green-600' : 'bg-slate-100 text-slate-600'" class="py-2 rounded-lg font-bold text-sm transition-all">
                  Shopee
